...
const MealTicketSystem = () => {
  ...
  useEffect(() => {
    ...
    script.async = true;
    script.onload = () => updateState({ jsQRLoaded: true });
    script.onerror = () => {
      const fallback = document.createElement('script');
      fallback.src = 'https://unpkg.com/jsqr@1.4.0/dist/jsQR.min.js';
      fallback.async = true;
      fallback.onload = () => updateState({ jsQRLoaded: true });
  ...
  const handleLogin = useCallback(async (e: React.FormEvent<HTMLFormElement> | React.MouseEvent<HTMLButtonElement> | React.KeyboardEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      updateState({ session: sessionData, mealUsedToday: false });
      changePage('dashboard');
      
    } catch (err) {
      console.error('Login error:', err);
      updateState({ error: 'Connection failed. Please check your internet connection.' });
    }
  ...
  const verifyMeal = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        updateState({ mealUsedToday: true });
        changePage('success');
      } else {
        updateState({ error: result.message || 'Failed to log meal' });
      }
    } catch (err) {
      console.error('Meal logging error:', err);
      updateState({ error: 'Failed to connect. Please try again.' });
    }
  ...
  const startQRDetection = useCallback(() => {
    ...
    animationFrameRef.current = requestAnimationFrame(detectFrame);
  }, [stopQRScanner, updateState, verifyMeal]);
